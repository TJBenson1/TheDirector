...
export function runOp(state: GameState, name: string, input: Record<string, unknown>): Op {
  ...
  switch (name) {
    ...
    case 'change_style': {
      ...
      const c = s.managerRelations;
      if (!verdict.applied) {
        // He dug in. Nothing changed — the Director can insist by calling again with force.
        return { state: s, result: { ok: false, resisted: true, coach: c.identity, reason: verdict.reason, canForce: true } };
      }
      return { state: s, result: { ok: true, coach: c.identity, style: c.archetype, formation: c.preferredFormation, relationship: c.relationshipWithUser, forced: verdict.forced, reason: verdict.reason } };
    }
...
export const TOOL_SCHEMAS = [
  ...
  { name: 'change_style', description: "Ask the CURRENT head coach to change his playing style and/or formation — the same man, a new approach. You MUST call this whenever the Director tries to get the coach to switch how the team plays (e.g. talking Capello into possession 4-3-3): narration alone does NOT change the game — only this tool updates the Manager's Room, the shape and the best XI. Call with no arguments to list the styles; then call with a style and/or formation. IMPORTANT: the coach can REFUSE — a dogmatic coach asked for a radical switch digs in, and the result comes back {resisted:true} with NOTHING changed. Relay his objection and let the Director decide: back off, or overrule him by calling AGAIN with force=true. Forcing it works but he plays it under protest and his happiness drops sharply. Do not set force=true on the first attempt — always let him have his say first.", input_schema: { type: 'object', properties: { style: { type: 'string', description: 'possession | gegenpress | pragmatic-counter | defensive-block | man-manager | balanced' }, formation: { type: 'string', description: 'e.g. 4-3-3, 4-2-3-1, 4-4-2' }, force: { type: 'boolean', description: 'Overrule a coach who has already refused. Only after a {resisted:true} result and the Director explicitly insists.' } }, required: [] } },
