/**
 * Curated, historically-accurate opening briefings — one per scenario.
 *
 * WHY: the opening meeting used to derive the coach's "best XI" from raw squad
 * ability, which produced formulaic, sometimes WRONG openings — it would seat
 * Roberto Baggio in Marcello Lippi's 1995 Juventus XI when the defining story of
 * that summer was Lippi freezing Baggio out and selling him to Milan. These
 ...
 * frozen-out, the sold, the wantaway, the wonderkid), and 2-4 sentences of
 * authored context unique to that summer. `coachBriefing` prefers this when a
 * scenario has an entry; otherwise it falls back to the ability-derived XI.
 *
 * `firstEleven` entries are "POSITION Name" display strings (GK/RB/CB/LB/DM/CM/RM/
 * LM/AM/RW/LW/CF/ST — a readable superset of the engine Position type).
 */
...
export interface ScenarioOpening {
  ...
  coach: string;
  formation: string;
  /** His genuine first-choice XI at the START of this season, positional order
   *  (GK -> forwards). Rendered instead of an ability-derived XI, so frozen-out
   *  or sold stars do not wrongly appear. */
  firstEleven: string[];
  /** The men NOT nailed on — frozen out, up for sale, wantaway, injured, or a
   *  wonderkid on the fringe — each with the real reason. */
  fringe?: string[];
  /** 2-4 authored, historically-accurate sentences of context for this summer. */
  briefing: string;
}
